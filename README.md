...
✅ Идентификация анализа по номеру исследования (имя файла)  
...
```
1. PDF файл → NAS (\\192.168.100.177\laba\123456.pdf)
2. Watcher обнаруживает файл
3. Integrator отправляет в 1С через HTTP API
4. 1С создаёт документ "Результат исследования"
5. 1С прикрепляет PDF к документу
6. 1С отправляет email пациенту (опционально)
7. Файл архивируется в /mnt/nas/archive/YYYY-MM-DD/
```
